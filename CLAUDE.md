...
**スクショは「画面の記録」ではなく「情報の伝達」。見る人が一目で内容を理解できる画像にする。**

- **撮影手順**: まず `screenshot` で全体を確認 → 伝えたい情報の範囲を特定 → `zoom` で必要な部分だけ切り取る
- **必ず除外するもの**: サイドバー、ナビゲーション、無関係な余白、ツールバー
- **判断基準**: 「この画像で何を伝えたいか」を考え、その情報だけが写るように範囲を決める

```
× screenshot でそのまま送る（余白・サイドバーが入って見にくい）
○ screenshot → 内容を確認 → zoom で必要な部分だけ切り取って送る
```

- **全体確認用**: `computer(action: "screenshot", tabId)`
- **切り取り（本命）**: `computer(action: "zoom", region: [x0, y0, x1, y1], tabId)`
- **貼り付け**: `upload_image(imageId: "返ってきたID", tabId, coordinate: [x, y])` または `upload_image(imageId, tabId, ref: "ref_xx")`

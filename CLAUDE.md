...
Codex で戦略・企画を練った後、Claude Code で実装する流れが基本。引き継ぎはファイル経由:

1. Codex が成果物をファイルに書き出す（例: `docs/` に企画書、`Project/` に設計書）
2. Claude Code がそのファイルを読んで実装に入る
3. 逆方向も同じ（Claude Code の実装結果を Codex がレビュー）

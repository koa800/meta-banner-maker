...
- フレックスメッセージ の新規作成は
  - `テンプレート -> 新しいテンプレート -> フレックスメッセージ`
  - route は `/line/template/edit_v3/new?group=0&editMessage=1`
...
- したがって live 作業では、最初に右上の account 表示を確認する
  - 2026-03-13 の current 実例では `スキルプラス【プライム】`
...
- さらに同日の live テストで、`画像` block も current UI で追加して保存まで確認した
  ...
  - `ファイルを選択する`
  - upload 後に一覧の file row を 1 回押して選択状態にする
  - `選択中のファイル数: 1 / 1`
  - modal の `保存`
  - editor 上部の `メッセージを保存`
  の順
...
- つまり、`新規作成時の URL` と `一覧行から再編集する URL` は一致しない前提で読む
...
- 一覧 API と種類判定を先に取りたい時は
  ...
  - `python3 System/scripts/lstep_template_catalog.py inspect --id 249006380`
...
4. 次の行動を 1 つに絞る
   ...
   - 予約
   - 回答フォーム
5. inline action で足りるか、reusable action に切り出すかを決める
6. 実際に `タップ -> 期待した画面 or 動作` を確認する
7. 必要ならタグやクロス分析で計測軸を足す
...
- `3月 スキルプラス合宿 集客`
  ...
  - 一覧からの current edit URL
    - `/line/template/edit_v3/249006380?group=0&editMessage=0`
  - alt
    - `【3月スキルプラス合宿】のご案内`
  - 構成
    - `1 panel`
...
- `マインドセットセミナー`
  ...
  - action
    - なし
  - 意図
    - `直前変更で不安な状態` を `変更を理解して次回参加に納得している状態` に変える
  - 良い点
    - `動画 + 画像 + テキスト` で感情補助と事実説明をまとめている
    - CTA を置かず、まず認識を揃えることに集中している
    - 重要告知を `フレックスメッセージ` にすることで、通常 text より視認性を上げている
...
- 2026-03-13 の live では、最小構成の `テキスト送信` action を end-to-end で確認した
  ...
  - `この条件で決定する`
  の順
  - `保存するアクション名` には visible textbox の 2 つ目を使う

...
- テンプレート一覧の current 正規入口は
  ...
  - つまり pack を経由しなくても、フレックスメッセージ単体を新規作成できる
...
- current editor は 1 画面で
  ...
  - `ボタン・カルーセル`
  を切り替える構造
- `ボタン・カルーセル` は `テンプレートパック内で Flex 的な表現を作る時の主入口` として読む
...
- 現時点で確定している事実は `単体 Flex の作成入口` と `pack 内カルーセル editor の field / action 種類` まで
- 画像必須条件や保存条件の最小構成は、代表例をさらに増やして詰める

#### 単体 Flex の current 作成とテスト送信
...
- create payload で最低限使われる field は
  ...
  - `twice_action_id`
- live で作成した隔離テストの実例
  - テンプレート名: `ZZ_TEST_20260312_Flexレビュー用`
  - id: `249263459`
  - 内容: title 1つ + text 2つの最小構成
- 単体 Flex のテスト送信は、editor 画面ではなくテンプレート一覧の行右端メニューから行う
  - 行右端の `more_vert`
  - `テスト送信`
  - `テスト送信先選択` ダイアログ
  - 友だちをチェック
...
- テンプレート: `https://manager.linestep.net/line/eggpack/show/249262717?group=`
- タグ: `https://manager.linestep.net/line/tag/setting/9615103?group=0`
- 流入経路: `https://manager.linestep.net/line/landing/edit/2351849`

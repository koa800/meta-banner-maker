...
- `テンプレート -> フレックスメッセージ`
  - `作成 -> 画像追加 -> テキスト追加 -> ボタン追加 -> アクション設定 -> 保存 -> テスト送信 -> 削除`
- `自動応答`
  - `新しい自動応答 -> 登録 -> 一覧確認 -> 削除`
- `回答フォーム`
...
- `アクション管理` の複合 pattern
- `リッチメニュー` の `作成 -> 保存 -> cleanup`
- `回答フォーム 2問以上` の representative
- high-quality template の構成と action 設計を current quality で再現すること
...
- SNS 流入は、集客媒体の前に `（アカウント名）` を付ける
  - 例: `（スキルプラス公式TV）YouTube _スキルプラス_直リストイン`
- `直リストイン` は `直接LINE登録したもの`
- `リストイン` は `メール登録後にLINE登録したもの`
...
- 先に `既存 family` を見るのではなく、次を順に確定する
  - `流入前オファーは何か`
  - `流入時の状態はどうか`
  - `その状態なら何のシナリオに入れるのが自然か`
- その後で、命名規則に沿って `流入経路分析` 名を決める
- つまり、`名前 -> 中身` ではなく `オファー / 状態 -> シナリオ -> 名前` の順で作る
- 2026-03-17 の `スキルプラス公式TV` 事例でも、この順に切り直して `無料体験会訴求シナリオ` を採用した

### 実案件の current 例

- 2026-03-17
  - YouTube `スキルプラス公式TV` 概要欄用に
    - `（スキルプラス公式TV）YouTube _スキルプラス_直リストイン`
    を `スキルプラス (@496sircr)` に作成
  - route では
    - route tag
    - `【新規】...`
    - 必須 tag
    ...
    - `無料体験会訴求シナリオ`
    を設定
  - short.io は `SNS(YT)` フォルダの `sp013`
- 既知の差分
  - 既存 family にある `友だち情報 [新規流入日] = action_date` 更新は current API で 422 になり、今回の route には未設定

### API で route を作る時の注意

- 既存 route の action を API でそのまま複製しても、必ずしも通らない
- 2026-03-17 の current では、`友だち情報 [新規流入日] = action_date` が 422 で落ちた
- したがって、API で作成した時は次を必ず見る
  - row 右側の action 説明が intended な内容か
  - 保存結果に欠損がないか
  - 必要なら 1 action ずつ UI 側と照合する

### `アクション管理` の current 挙動

- `新しいアクション` は別ページ遷移ではなく、一覧右側に `アクション設定` の side drawer を開く
- drawer では、先に `保存するアクション名` を入れ、その後 `行う動作を選択してください` から 1 動作目を選ぶ
- 2026-03-17 の current で exact に確認できたラベル
  - `保存するアクション名`
  - `フォルダ`
  - `行う動作を選択してください`
  - `テキスト送信`
  ...
  - `コンバージョン操作`
  - `この条件で決定する`
- `タグ操作` は、タグ名を文字で打つだけでは通らない
  - current UI では `タグ選択` が主入口
  - `タグ選択` を通さずに `この条件で決定する` を押すと
    - `登録に失敗しました`
    - `1.[タグ編集]タグは必須です。`
    が表示される
- つまり `タグ操作` の exact 手順は
  - `タグを追加 / タグをはずす`
  - 必要なら `タグフォルダで指定`
  - `タグ選択`
  - `この条件で決定する`
  の順で見る
- unsaved な drawer を閉じる時は confirm が出る
  - `保存されていない変更があります。本当に閉じますか？`
  - `キャンセル / 閉じる`

### 入口選択の exact な問い

Lステップで迷いやすいのは、`何を送りたいか` ではなく `どの画面から作るべきか` です。最初に次を切る。

- 今やりたいのは `単発の message 作成` か
  - なら `テンプレート`
- 今やりたいのは `同じ message を順番に流す` か
  - なら `シナリオ配信`
- 今やりたいのは `1回だけ広く送る` か
  - なら `一斉配信`
- 今やりたいのは `特定の入力や応答に反応して返す` か
  - なら `自動応答`
- 今やりたいのは `情報回収` か
  - なら `回答フォーム`
- 今やりたいのは `状態をまとめて切り替える` か
  - なら `アクション管理`
- 今やりたいのは `入口を常設する` か
  - なら `リッチメニュー`

つまり Lステップでは、最初の 1 手は `message を作るか` ではなく、
...
- `フレックスメッセージ`
  - 1画面で重要度の差を見せたい
  - 画像とボタンで main CTA を強く見せたい
- `テンプレートパック`
  - 1通ではなく message 群として扱いたい

この 10 秒判断が言えない時は、`テンプレート` 一覧で新規作成に入らない。

### live 作業での session mismatch

Lステップでは、次の状態が起きる。

- 画面上はログイン済みに見える
- 一覧や編集画面は開ける
- でも `保存` や `テスト送信` の API が `401` になる

この時は `ログインしているように見えるから続行` が一番危険。  
正しい判断は、`browser の見た目の session と API session がずれている` です。

### live 作業での account context mismatch

Lステップでは、次の別パターンも起きる。

- `python3 System/scripts/lstep_auth.py` は `auth_alive=true`
- でも `account_name` が intended な公式LINE account ではない
- API 一覧は `200` で返る
- ただし `data=[]` で、`回答フォーム` や `アクション管理` や `テンプレート` が空に見える

2026-03-16 の current では、`auth_alive=true` でも `account_name=koa800sea` を返した。  
つまり `認証が生きている` と `今触りたい公式LINE account を見ている` は別物です。

#### 復旧順

1. `python3 System/scripts/funnel_tool_live_readiness.py`
2. `lstep.auth_alive=true` を確認
3. `lstep.account_name` が intended account か確認
4. 違う時はその文脈で作業を続けない
5. intended account に切り替えた後で、もう一度 readiness を取り直す

#### 復旧順

1. その場で保存を連打しない
2. `https://manager.linestep.net/account/login` に戻る
3. 手動で再ログインする
4. `テンプレート` や `シナリオ配信` の対象行へ入り直す
5. `保存` または `... -> テスト送信` を再試行する
...
- したがって current の開始条件は
  1. `python3 System/scripts/funnel_tool_live_readiness.py`
  2. `lstep.auth_alive=true`
  3. `lstep.account_name` が intended account
  4. その後に `フレックスメッセージ` の保存や `テスト送信`
  で固定する
- `auth_alive=false` の時は、その場で browser 手動ログインを依頼する
- `auth_alive=true` でも `account_name` が違う時は、認証切れではなく account 文脈ズレとして扱う
...
> スプレッドシート: [【アドネス株式会社】マスタデータ / Lステップ](https://docs.google.com/spreadsheets/d/1kxUbLqhnzLC1Pg0ASVgU135bnx4Rsv_jP0pqGC0R69w/edit?gid=639318025#gid=639318025)
> 詳細分析は `Master/addness/lstep_accounts_analysis.md` を参照

このタブはマスタデータ上で `稼働中` と `停止中` を同じ一覧として管理する。正本として扱うのは `稼働状況 = 稼働中` の行だけで、`停止中` の行は旧アカウントや誤接続候補の確認用とする。
...
- current 一覧 route は `https://manager.linestep.net/line/reaction`
- current router 定義では
  - 一覧: `/line/reaction`
  - 新規: `/line/reaction/new`
  - 編集: `/line/reaction/edit/{id}`
- current 一覧の visible 構造は次
  - 上部 button
    - `新しい自動応答`
    - `一括操作`
    - `CSVアップロード`
  ...
  - row 右端 menu
    ...
    - `ONにする / OFFにする`
    - `削除`
- current 一覧 API は `https://manager.linestep.net/api/reactions?page=1`
  - row では少なくとも次を読む
    - `name`
    - `keywords`
    - `action_text_array`
    ...
    - `holiday_rule`
- current folder API は `https://manager.linestep.net/api/reaction-groups`
- current 実例として `【予約窓口】スキルプラス合宿` 側では
  - `希望時間23:00~`
  - `希望時間22:00~`
  - `希望時間21:00~`
  のような `キーワード -> 友だち情報代入` 型が並んでいた
  - つまり `自動応答` は、単なる自動返信だけでなく、予約導線の状態入力にも使われる
- 2026-03-14 の live 最小往復では
  - `新しい自動応答`
  ...
  まで exact に確認した
- したがって reusable 部分は `Skills/2_導線/lstep-auto-response-operations` に昇格させる
- Addness 固有では
  - `自動応答` は `返信` だけでなく `状態更新` にも使う
  - current 例では `希望時間XX:00~` の予約時間入力型が strong representative
 という読み方を `Master` 側に残す
...
- このアクションは `状態整理 / 送信トリガー / シナリオ制御` のどれか
- 何を 1 つに束ねるのか
- reusable にする価値があるか
- 次の state を 1 文で説明できるか
- inline で十分か、`アクション管理` に切り出すべきか
...
- current の代表例は、まず `構築方法` の benchmark として読む
- つまり
  - どの message 種を使うか
  - 画像をどう置くか
  - テキストをどう分けるか
  ...
  - `削除`
  を exact に学ぶ材料
- 一方で、`見た目や訴求の品質上限` を current 例そのものに置かない
- 少なくとも 2026-03-15 時点では、current の `フレックスメッセージ` は
  - 構築の参考にはなる
  - ただし品質 benchmark としては 60 点台のものも含む
  という前提で読む
- したがって、既存例からは
  - exact 手順
  - action 設計
...
- このリッチメニューは `入口分岐 / テンプレート送信 / 回答回収` のどれか
- 誰向けの入口か
- どの action に分業するか
- 責務を載せすぎていないか

...
- 既存の representative 例として、`3月 スキルプラス合宿 集客` では
  ...
  - button 1 block
  - image 1 block
  が組まれており、button の `アクション設定` は `URLを開く` だった
...
- 後片付けの current 手順も確認済み
  - テンプレート一覧の行右端 `more_vert`
  - `削除`
  - confirm dialog `この操作は取り消せません。本当に削除しますか？`
  - `削除する`
- 2026-03-12 の live テストでは、作成した一時 `フレックスメッセージ` を
  - `ボタン` block 追加
  - `URLを開く` 追加
  - `画像` block 追加
  - `甲原 海人` へのテスト送信
  - `削除する`
  まで通し、その後一覧に残っていないことを確認した
...
- 一覧画面では
  ...
  - `テスト送信`
  が主な入口
- pack 内の step は current では少なくとも
  - text editor 系
  - `ボタン・カルーセル`
  の family を持つ
- representative 実例から見る current の意図は
  - `無料相談会告知` のように、複数 message を 1 セットとして回す
  - 1通単体より、同テーマの message 群を再利用したい時に使う
  - `2026/01/15架電テンプレ(小笠原)`
  を確認した
  - 構成は `本文テキストのみ`
  - 本文中に direct LIFF をそのまま入れている
  - `img_media_id / audio_media_id / video_media_id / pdf_media_id` は空
  - つまり current の標準メッセージは、少なくとも `スキルプラス【サポートLINE】` では `長めのテキスト連絡` が主戦場
- 2026-03-12 の live テストでは
  - 一時 `標準メッセージ`
  - item id `249266659`
  - `甲原 海人` への `テスト送信`
  - 一覧からの削除
  まで通し、一覧に残っていないことを確認した
- `テスト送信` は create 画面内にもあるが、current の安全手順としては一覧に戻ってからのテスト送信に寄せる
- 実行時の exact endpoint は
  - `POST /line/template/test/multi`
  - payload `item_ids[]=249266659`, `tester_ids[]=130515`
  - 応答 `テストアカウントへ送信しました。`
- 削除は generic template endpoint で通った
  - `DELETE /api/templates/249266659`
  - status `204`
...
- save request は
  ...
  - `eggjson` 内に
    ...
    が入る
- 2026-03-12 の live create では、最小構成の action type は `4` で保存された
...
- 2026-03-12 の live 実行では
  - template item id `249264109`
  - tester id `130515`
  - `POST /line/template/test/multi`
  - payload `{\"item_ids\":[249264109],\"tester_ids\":[130515]}`
  - 応答 `テストアカウントへ送信しました。`
  まで確認した
- 削除の current 手順は
  1. 対象行の右端 `more_vert`
  ...
  4. `削除する`
- 2026-03-12 の live 実行では
  - `DELETE /api/templates/249264109`
  - 一覧上 `exists=0`
  まで確認した

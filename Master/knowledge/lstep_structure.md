...
- フレックスメッセージ の新規作成は
  - `テンプレート -> 新しいテンプレート -> フレックスメッセージ`
  - route は `/line/template/edit_v3/new?group=0`
...
- 2026-03-12 の live create では、`スキルプラス【サポートLINE】` 上で
  - `group=1`
  - `sender_id=1`
  で保存成功を確認した
...
- フレックスメッセージ のテスト送信は、editor 画面ではなくテンプレート一覧の行右端メニューから行う
  ...
  - 友だちをチェック
  - `テスト`
- 2026-03-12 の live 実行では、送信先に `甲原 海人` を選択し、
...
- 後片付けの UI 導線はここまで確認済み
  ...
  - `削除する`
- ただし `単体フレックスメッセージ` の削除は、後日の一覧スクリーンショットで `ZZ_` テスト資産が残っていることを確認した
  - したがって `削除 UI の導線は分かっている`
  - だが `フレックスメッセージの live delete が完全に再確認済み` とまでは扱わない
  - `作成 -> テスト送信` は確認済み
  - `削除後に一覧 / API の両方で消失確認` は次回 live session で再確認する
...
- 2026-03-12 の live create では
  - pack id `249326924`
  - pack 名 `ZZ_TMP_20260312_pack`
  まで確認した
...
- 最小の `本文` step は
  ...
  - 最低限 `text_text` を入れて保存できた
- 2026-03-12 の live create では
  - step 本文 `ZZ test pack body`
...
- 後片付けの exact 手順も確認した
  - step 削除
    - `POST /line/eggpack/step`
    - `_method=DELETE`
    - `hole_id=13453796`
  - pack 削除
    - `DELETE /api/templates/249326924`
  - `GET /api/templates` 上で `exists=0`
...
- `3月 スキルプラス合宿 集客`
  ...
  - editor_json で分かること
    ...
    - `action.data.type = 1`
    - `action.data.url_action.url` に遷移先が入る
    - `action.data.url_action.liff_size = full`
    - description は `[URLを開く] ... をトーク内ブラウザ(大)で開きます`
  - action
    - `詳細はこちら`
    - 実体は `URLを開く`
...
- `新しいアクション` を最小構成で新規保存して削除するところまでは、この session では未固定
- 原因は二段階ある
  - `タグ操作 -> タグ選択` の selector で候補の選び方が current UI 上まだ甘く、save 前の exact click path を追加で取る必要がある
  - 2026-03-12 時点では全 cookie source が `401 Unauthorized` で、login page は `reCAPTCHA enabled=true`
- したがって `アクション管理` は
  - 既存 action を読む
  - field と save route を理解する
  ところまでは exact
  - ただし `新規作成 -> 保存 -> 削除` の完全な end-to-end は次回の live 固定対象

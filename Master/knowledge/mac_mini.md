...
### Python ランタイム方針
- Mac Mini の正本 Python は `~/agent-env/bin/python3`。`setup_phase1.sh` が Homebrew `python@3.12` で明示的に作る
- 起動・監視・セットアップは `System/scripts/python_runtime.py` で 3.10+ を優先解決する
- `run_agent.sh` / `install_orchestrator.sh` / watchdog / Addness パイプラインはこの resolver 経由で Python を選ぶ
- 3.10 未満の仮想環境が残っている場合は `setup_phase1.sh` の再実行で作り直す
- まとめて修復する時は `bash System/mac_mini/repair_python_runtime.sh` を使う
- MacBook から直接流す時は `bash System/mac_mini/run_remote_python_runtime_repair.sh` を使う
...
- Python ランタイム診断: `python3 System/scripts/python_runtime_status.py`
- Python ランタイム修復: `bash System/mac_mini/repair_python_runtime.sh --dry-run`
- リモート修復: `bash System/mac_mini/run_remote_python_runtime_repair.sh --dry-run`
...
- 運用時は system Python 固定ではなく、`~/agent-env/bin/python3` を最優先で使う

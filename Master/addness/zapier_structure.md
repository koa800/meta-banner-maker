...
- exact に読めているもの
  ...
  - `Assets > Zaps > 甲原 > Create Zap -> Trigger > Webhooks by Zapier > Catch Hook -> Action > Mailchimp > Add/Update Subscriber -> Test -> Delete Zap`
    を 1 action relay で live exact に完了できること
  - `Assets > Zaps > 甲原 > Create Zap -> Trigger > Webhooks by Zapier > Catch Hook -> Action > Webhooks by Zapier > POST -> Test -> Delete Zap`
    を 1 action relay で live exact に完了できること
...
- create 入口: `Assets > Zaps > 甲原 > Create Zap`
...
- current の exact 挙動は、folder page で `Create Zap` を押してから `Untitled Zap / Draft` builder へ入る

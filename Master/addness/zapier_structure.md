...
- exact に読めているもの
  ...
  - `Assets > Zaps > Create -> Trigger > Webhooks by Zapier > Catch Hook -> Action > Mailchimp > Add/Update Subscriber -> Test -> Delete Zap`
    を 1 action relay で live exact に完了できること
  - `Assets > Zaps > Create -> Trigger > Webhooks by Zapier > Catch Hook -> Action > Webhooks by Zapier > POST -> Test -> Delete Zap`
    を 1 action relay で live exact に完了できること
  - 2026-03-17 の current assets list では、`Create` を押すとそのまま builder に入る
  - success 条件は `Create` が押せたかではなく、`New Zap | Zapier` と `Trigger 1. Select the event that starts your Zap` が見えているかに置く
  - folder page 側の `Create -> Create Zap` は不安定で、ghost draft の誤読が起きうる
...
- `trigger 1 + action 2` の exploratory probe は current で live exact に通る
...
- `python3 System/scripts/zapier_create_delete_probe.py --with-action --with-second-action --action-app webhook-post --second-action-app mailchimp`
  ...
  - `post_second_action_stage = Test`
  - `deleted = true`
- 一方で folder page 側の `Create -> Create Zap` は builder に入らない回がある
  - この回は selector 問題より、ghost draft や stale link を先に疑う
  - `Choose an event` が見えない時は、`New Zap | Zapier` と builder URL を再確認する
- current builder 上では first action 後に
  - `Choose an event`
  - `Select`
  - `Add account to continue`
  が visible
- 2026-03-17 の `zapier_second_action_probe.py` では
  - first action を `Webhooks by Zapier -> POST` で `Test` stage まで進めた後
  - before:
    - URL = `.../draft/.../setup`
    - step node = `1. Catch Hook / 2. POST`
    - body marker = `Action / Test / Publish`
  - second action 入口を開くと
  - URL は `.../setup` から `.../fields` へ遷移
  - after:
    - step node は `1. Catch Hook / 2. POST` のまま
    - `Add a step` は visible marker に出ていない
  - つまり `2つ目の action picker` ではなく `既存 action の fields` を再度開いている
- current の実 UI では、`Add a step` は visible text ではなく `aria-label="Add step"` の button として出る
- `zapier_create_delete_probe.py` は 2026-03-17 にこの selector を拾うよう更新済み
- その結果、`trigger 1 + action 2` の live create / test / delete まで exact に通った
...
- 残差は representative family を増やすことと、既存 relay 修正時の rollback 本数を積むこと
...
- create 入口: `Assets > Zaps > Create`
- create 入口の folder URL: `https://zapier.com/app/assets/zaps/folders/019cdd75-b612-ec46-7e5b-bd8a9015a667`
- exact には、assets list を開いて `Create` を押し、そのまま builder へ入る
- create 後は current builder へ遷移し、内部的には `webintent` 系 URL を経由することがある
...
- current の exact な新規作成入口は `https://zapier.com/app/assets/zaps/folders/019cdd75-b612-ec46-7e5b-bd8a9015a667`
- 2026-03-17 の live probe では、この `甲原` フォルダ画面から `Create Zap` した draft が assets 一覧 row で `Location = 甲原` になることを確認済み
...
- current 実動作では
  ...
  を JSON 出力できることを確認済み
- `python3 System/scripts/zapier_family_snapshot.py --limit 5`
- `Playwright.connect_over_cdp` が timeout する回でも raw fallback で family を再取得できる
- current では
  ...
  を再確認済み
- `steps=[]` で `signature=""` の `Untitled Zap` は draft 候補として family 集計の意味読みから外す

### create 入口で分かること

- folder page 側の `Create` は current では不安定で、作成メニューの入口として出る回がある
- したがって current の一次正本は `Assets > Zaps > Create`
- `Create` が押せたこと自体ではなく、builder に入ったことを success 条件にする
- `Create Zap` から入ると、最初は `Untitled Zap / Draft` の builder が開く
- 初期 builder には
  - `Trigger`
  - `Action`
...
- build 中に内部遷移で `https://zapier.com/webintent/create-zap?...` が見えても、current の主入口とはみなさない
- folder page を開いただけでは builder には入らない
- current の exact 挙動は、assets list で `Create` を押すと `Untitled Zap / Draft` builder へ入る
- 2026-03-17 の current probe では、folder page からでも `https://zapier.com/editor?...` に直接入る回がある
- その時の exact 初期ラベルは
...
- 2026-03-17 の current probe では、folder page に `Untitled Zap` link が visible でも、editor 側は `/500` で開けず `zapier_cleanup_untitled.py --edit-path` は `before_count=0, deleted_count=0, after_count=0` を返す回があった
- つまり folder page の `Untitled Zap` link は stale / ghost draft の可能性がある
- この回は `visible link がある = persisted draft がある` と決め打ちせず、assets list / cleanup script / editor 応答の 3 点で判定する
- 2026-03-16 live では
...
- 2026-03-17 の current app picker では、trigger app の候補表示が `Webhooks by Zapier` ではなく `Webhooks Premium` だった
- したがって current の exact 読みでは
  - internal family 名: `Webhooks by Zapier`
  - app picker 表示名: `Webhooks Premium`
  を分けて扱う
...
- title / menu
  ...
  - current UI では exact text 一致より、`Untitled Zap` を含む button として拾う方が安定
  - current menu は
    - `Rename`
    - `Duplicate`
    - `Transfer data`
...
- folder move
  ...
  - folder picker で
    ...
    が出る
  - `甲原` は picker に出る
  - 2026-03-17 の current probe では、`甲原` を選んでも assets 一覧の `Location` へ即反映されなかった
  - したがって `Folder = 甲原` は Addness の運用ルールとして維持しつつ、current UI では exact-confirmed ではない
  - 一方で 2026-03-17 の current probe では、`甲原` フォルダ画面から `Create Zap` した draft は assets 一覧 row で `Location = 甲原` を満たした
  - したがって current の exact 運用では、`Zap details > Folder` を後から変えるより、`甲原` フォルダ直下 `Create Zap` を優先する
- delete
  - 左上の title menu
  - `Delete`
  - confirm dialog

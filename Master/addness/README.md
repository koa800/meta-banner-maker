...
この再採点の理由
- Lステップ
  - `フレックスメッセージ` に加えて `標準メッセージ` と `カルーセルメッセージ(新方式)` も
    - 作成
    - `甲原 海人` への `テスト送信`
    - 削除
    まで exact に通った
  ...
  - ただし代表例の厚みや hidden / legacy の判断辞書はまだ増やせる
- UTAGE
...
- Zapier
  ...
  - `Create` 入口は `Untitled Zap / Draft / sandbox` から始まり、`Webhooks by Zapier -> Catch Hook` と `Mailchimp -> Add/Update Subscriber` の選択肢まで exact に取れた
  - `Folder` は `Zap details` から管理するところまで取れた
  - ただし title 変更と publish までの exact 再現はまだ不足

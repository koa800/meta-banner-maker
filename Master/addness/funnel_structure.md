...
- シート構成は `はじめに`、`広告（みかみ導線）`、`広告（スキルプラス導線）`、`広告（ライトプラン導線）`、`広告（書籍）`、`SNS`、`SEO`、`広報`、`その他`
- 基本の列構成は `カテゴリ / 設置場所 / リンクタイトル / URL / 遷移先 / 最新フラグ`
- `はじめに` シートには短縮リンク作成 Loom、検索用フィルタ（例: `title:YouTube`）、固定リンクを優先すべき設置箇所のメモがある
...
1. active queue がある AI / SNS の現役ページで、`停止中 LIFF` と `稼働中 LIFF / short.io` のどちらが実際の主 CTA なのかの最終判定
2. AI 側で `みかみ@AI_個別専用` と `【みかみ】アドネス株式会社` がページごとに分かれている理由と、どちらが正本かの確定
3. SNS 側で `みかみ@個別専用` と `【みかみ】アドネス株式会社` がページごとに分かれている理由と、どちらが正本かの確定
4. LINE着地後の Lステップ 内で、どのタグ / シナリオ / 個別予約導線に入るか
5. short.io の JWT 復号による API 直取得
6. スキルプラス導線の `相談会 / 体験会` 側の現行主線の切り分け
7. SNS / スキルプラスの current Zap を Zapier 側で再度引き切ること
8. デザイン / 広告ファネルの行き先LINE（現状は優先度低）

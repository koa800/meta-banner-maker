...
| タブ名 | 内容 |
|---|---|
| 個別予約通知ログ | `予約イベント日時 / LINE名 / LSTEPメンバーID / Lステップアカウント名 / Lステップリンク / メールアドレス / 電話番号`。`個別予約データ / シート1` の過去データと `Slack #個別予約通知` の bot 通知を 1イベント1行で統合する。手元の友だち一覧CSVは使わず、`Lステップリンク` から会員詳細を live 取得して `メールアドレス / 電話番号` を補完する |

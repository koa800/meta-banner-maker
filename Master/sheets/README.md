...
| `1ip_RARDHmQvTjmaVavw1L71ltPrn4Kg6sa__njqyQZ8` | 【アドネス株式会社】個別面談データ | [リンク](https://docs.google.com/spreadsheets/d/1ip_RARDHmQvTjmaVavw1L71ltPrn4Kg6sa__njqyQZ8/edit) | 個別予約の第1版加工シート。`2026/03/16` 以前の `日別個別予約数` は `【アドネス】顧客管理シート / 個別予約集計botログ` を使い、`2026/03/17` 以降は `個別予約データ / シート1` の過去データと `Slack #個別予約通知` を統合した `個別予約通知ログ` で補完する。通知ログを使う期間は、同一人物の通知が同じ日に10分以内で連続した場合は1件として扱う。`日別個別予約数 / 日別個別予約数（UU） / 個別予約サマリー / タグ検証 / 個別予約通知ログ / データソース管理 / データ追加ルール` を持つ。現時点で接続するのは `個別予約数` のみで、`個別予約数（UU）` と `個別実施数` は将来接続 | `python3 System/booking_metrics_sheet_sync.py` と `python3 System/booking_notification_log_sync.py` で更新 |

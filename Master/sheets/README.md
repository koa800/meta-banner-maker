...
| タブ名 | 内容 |
|---|---|
| 日別メール登録件数 | `日付 / メール登録件数 / 累計メール登録件数`。`2025/01/01 - 2026/03/06` は `Master/output/email_collection_metrics_history_snapshot.json` の固定値を継承し、`2026/03/07` 以降は `【アドネス株式会社】集客データ（メールアドレス）` を正本にして重複ありで集計。`【アドネス株式会社】共通除外マスタ` の `除外リスト / 無条件除外ルール` を参照し、追加日以降の新規集客データだけ除外する |
| 日別メール登録件数（UU） | `日付 / メール登録件数（UU） / 累計メール登録件数（UU）`。`2025/01/01 - 2026/03/06` は `Master/output/email_collection_metrics_history_snapshot.json` の固定値を継承し、`2026/03/07` 以降は `【アドネス株式会社】集客データ（メールアドレス）` で最初に確認された日にだけ 1件と数える。共通除外マスタの条件も同時に適用する |

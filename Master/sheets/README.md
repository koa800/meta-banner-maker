...
| `12bYadR0cgi24t4tz8GeESlsKffmNkkTHprI4ray_Sq4` | 【アドネス株式会社】個別面談データ（収集） | [リンク](https://docs.google.com/spreadsheets/d/12bYadR0cgi24t4tz8GeESlsKffmNkkTHprI4ray_Sq4/edit) | 個別予約の収集データシート。`個別予約通知ログ / データソース管理 / データ追加ルール` を持ち、`Slack #個別予約通知` を継続取得の正本として `1イベント1行` で保存する。列は `予約イベント日時 / LINE名 / LSTEPメンバーID / Lステップアカウント名 / Lステップリンク / メールアドレス / 電話番号`。過去データは初回移行時のみ統合し、継続取得の正本にはしない。取りこぼし防止のため、毎回直近数日の Slack 通知を再取得して重複吸収する | `python3 System/booking_notification_log_sync.py` で更新 |
...
| タブ名 | 内容 |
|---|---|
| 個別予約通知ログ | `予約イベント日時 / LINE名 / LSTEPメンバーID / Lステップアカウント名 / Lステップリンク / メールアドレス / 電話番号`。`個別予約データ / シート1` の過去データと `Slack #個別予約通知` の bot 通知を 1イベント1行で統合する。手元の友だち一覧CSVは使わず、将来は `Lステップリンク` から live 取得で `メールアドレス / 電話番号` を補完する |
| データソース管理 | 個別予約通知ログの継続取得元、最終同期日、更新数、`正常 / 未同期 / 停止` のステータス、メモを持つ |

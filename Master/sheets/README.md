...
| 個別予約通知ログ | `予約イベント日時 / LINE名 / LSTEPメンバーID / Lステップアカウント名 / 通知リンク / メールアドレス / 電話番号`。`個別予約データ / シート1` の過去データと `Slack #個別予約通知` の bot 通知を 1イベント1行で統合し、LSTEPメンバーID で友だち一覧CSVに一致した時は `メールアドレス / 電話番号` も自動補完する |

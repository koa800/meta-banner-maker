...
| タブ名 | 内容 |
|---|---|
| 個別予約通知ログ | `予約イベント日時 / LINE名 / LSTEPメンバーID / Lステップアカウント名 / メールアドレス / 電話番号`。`個別予約データ / シート1` の過去データと `Slack #個別予約通知` の bot 通知を 1イベント1行で統合する |

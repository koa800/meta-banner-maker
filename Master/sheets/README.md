...
| `1dSIXBovs-c8wVnBWsOqbe2wdqmJQ10bOIWhKJbC1MPw` | 【アドネス株式会社】共通除外マスタ | [リンク](https://docs.google.com/spreadsheets/d/1dSIXBovs-c8wVnBWsOqbe2wdqmJQ10bOIWhKJbC1MPw/edit) | 共通除外の正本。`除外リスト / データソース管理 / データ追加ルール` を持ち、集客・個別予約・決済などの新規データだけを共通除外する。候補元は `支払管理表 / 全メンバーリスト`、正本はこのシートの `除外リスト`。`test / テスト / sample / dummy` など明らかなテストデータは共通ルールで無条件除外する | `python3 System/common_exclusion_master_setup.py` で再生成 |
...
| タブ名 | 内容 |
|---|---|
| 日別メール登録件数 | `日付 / メール登録件数 / 累計メール登録件数`。`2025/01/01 - 2026/03/06` は `Master/output/email_collection_metrics_history_snapshot.json` の固定値を継承し、`2026/03/07` 以降は `【アドネス株式会社】集客データ（メールアドレス）` を正本にして重複ありで集計 |
| 日別メール登録件数（UU） | `日付 / メール登録件数（UU） / 累計メール登録件数（UU）`。`2025/01/01 - 2026/03/06` は `Master/output/email_collection_metrics_history_snapshot.json` の固定値を継承し、`2026/03/07` 以降は `【アドネス株式会社】集客データ（メールアドレス）` で最初に確認された日にだけ 1件と数える |
| 複数アドレスユーザー | `顧客ID / メールアドレス数 / メールアドレス`。`【アドネス株式会社】顧客データ（複数イベント） / 顧客マスタ` を参照して、1人で複数メールを持つ顧客を確認する |
...
| タブ名 | 内容 |
|---|---|
| 日別個別予約数 | `日付 / 個別予約数 / 累計個別予約数`。`【アドネス】顧客管理シート / 個別予約集計botログ` のうち、`日付あり and キャンセル=FALSE` の行だけを日別集計 |
...
| 個別予約通知ログ | `予約イベント日時 / LINE名 / LSTEPメンバーID / Lステップアカウント名 / 通知リンク / メールアドレス / 電話番号`。`個別予約データ / シート1` の過去データと `Slack #個別予約通知` の bot 通知を 1イベント1行で統合する。現状は `★【個別予約完了】★` のタグ通知を取り込み、将来は `salon_reservation_reserved / reservation.reserved` の予約イベント通知も同じ正本へ広げる |
...
- 通知ログ更新: Orchestrator が `43 */2 * * *` で `Slack #個別予約通知` を読み込み、`個別予約通知ログ` を更新する

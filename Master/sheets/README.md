...
| 個別予約通知ログ | `タグ付与日時 / LINE名 / LSTEPメンバーID / Lステップアカウント名 / 予約導線種別 / 通知リンク / メールアドレス / 電話番号`。`Slack #個別予約通知` の bot 通知を 1イベント1行で保存し、LSTEPメンバーID で友だち一覧CSVに一致した時は `メールアドレス / 電話番号` も自動補完する |

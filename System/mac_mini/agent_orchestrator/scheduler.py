...
_MODULE_SYSTEM_DIR = Path(__file__).resolve().parents[2]
_MODULE_PROJECT_ROOT = _MODULE_SYSTEM_DIR.parent
_RUNTIME_PROJECT_ROOT = Path(
    os.environ.get("ADDNESS_DEPLOY_ROOT", str(_MODULE_PROJECT_ROOT))
).expanduser().resolve()
_RUNTIME_SYSTEM_DIR = _RUNTIME_PROJECT_ROOT / "System"
_RUNTIME_MASTER_DIR = _RUNTIME_PROJECT_ROOT / "Master"
...
_CDP_SYNC_NOTIFY_STATE_FILE = _RUNTIME_DATA_DIR / "cdp_sync_notify_state.json"


def _ensure_system_path() -> None:
    """System/ 配下のモジュールを import できるようにする。"""
    system_dir_str = str(_MODULE_SYSTEM_DIR)
...
def _save_json_file(path: Path, payload: dict) -> None:
    try:
        ...
        logger.warning(f"json save failed: path={path} error={e}")


def _is_cdp_success_message(message: str) -> bool:
    return bool(message) and message.startswith("CDP同期が終わりました")


def _should_send_cdp_success_notification(now: datetime) -> bool:
    state = _load_json_file(_CDP_SYNC_NOTIFY_STATE_FILE, {})
    return state.get("last_success_date") != now.date().isoformat()


def _mark_cdp_success_notification_sent(now: datetime) -> None:
    _save_json_file(_CDP_SYNC_NOTIFY_STATE_FILE, {
...
def _build_execution_rules_compact() -> str:
    ...
    candidates = [
        _RUNTIME_MASTER_DIR / "learning" / "execution_rules.json",
        _RUNTIME_PROJECT_ROOT / "_repo" / "Master" / "learning" / "execution_rules.json",
        _MODULE_PROJECT_ROOT / "Master" / "learning" / "execution_rules.json",
    ]
    rules = []
    for p in candidates:
        try:
...
class TaskScheduler:
    def __init__(self, config: dict, memory: MemoryStore):
        ...
        self.project_root = Path(
            paths.get("project_root") or paths.get("repo_root") or str(_RUNTIME_PROJECT_ROOT)
        ).expanduser()
        self.system_dir = Path(paths.get("system_dir", str(_RUNTIME_SYSTEM_DIR))).expanduser()
        self.master_dir = Path(paths.get("master_dir", str(_RUNTIME_MASTER_DIR))).expanduser()
        self.runtime_data_dir = _RUNTIME_DATA_DIR
        self.credentials_dir = self.system_dir / "credentials"
        self.hinata_dir = self.system_dir / "hinata"
        self.scheduler = AsyncIOScheduler(timezone=_SCHEDULER_TIMEZONE)
        self._task_map = {
            "addness_fetch": self._run_addness_fetch,
            "addness_activity_watch": self._run_addness_activity_watch,
            "addness_smoke_test": self._run_addness_smoke_test,
            ...
            "security_action_watch": self._run_security_action_watch,
    ...
    async def _run_addness_activity_watch(self):
        result = await self._execute_tool(
            "addness_activity_watch",
            tools.addness_activity_watch,
            pages=5,
            page_size=100,
        )
        if result.success:
            logger.info("Addness activity summary updated")

    async def _run_addness_smoke_test(self):
        result = await self._execute_tool(
            "addness_smoke_test",
            tools.addness_smoke_test,
        )
        if result.success:
            logger.info("Addness smoke test completed")
    ...
    async def _run_security_action_watch(self):
        result = await self._execute_tool("security_action_watch", tools.security_action_watch)
        ...
        try:
            ...
            logger.warning(f"security_action_watch returned non-json output: {result.output[:200]}")
            return

        if not payload.get("new_id_detected"):
            return

        from .notifier import send_line_notify

        self_id = payload.get("self_declaration_id", "")
        subject = payload.get("subject", "")
        message = (
            "補助金の前提が1つ進みました。\n"
            f"SECURITY ACTIONの自己宣言IDを検知しました: {self_id}\n"
            f"件名: {subject}\n"
            "次はIT導入補助金の対象ツール精査に進めます。"
        )
    ...
    async def _run_ds_insight_weekly_digest(self):
        ...
        pool_path = self.system_dir / "data" / "ds_insight_weekly_pool.json"
...
- 全体として簡潔に。長くても15行以内""",
    ...
    def _update_market_knowledge(self, emails: list, weekly_summary: str, week_date: str):
        ...
        knowledge_path = self.master_dir / "addness" / "market_trends.md"
...
{weekly_summary}"""}]
    ...
    async def _run_ds_insight_biweekly_report(self):
        ...
        # 前回データ読み込み
        data_path = self.system_dir / "data" / "ds_insight_last.json"
...
===RESULT_END==="""
    ...
    async def _run_health_check(self):
        ...
        qa_state_path = self.runtime_data_dir / "qa_monitor_state.json"
        if qa_state_path.exists():
            try:
        ...
        kpi_cache = self.system_dir / "data" / "kpi_summary.json"
        if kpi_cache.exists():
            try:
                import time
    ...
    async def _run_daily_addness_digest(self):
        ...
        goal_tree_path = self.master_dir / "addness" / "goal-tree.md"
    ...
    async def _notify_today_calendar(self, send_line_notify):
        ...
        try:
            ...
            profiles_path = self.master_dir / "people" / "profiles.json"
            profiles = {}
            try:
    ...
    async def _run_render_health_check(self):
        ...
        import json as _json
        import urllib.request
        from .notifier import get_line_notify_config, send_line_notify

        server_url, _, _ = get_line_notify_config()
        try:
    ...
    async def _run_oauth_health_check(self):
        ...
        token_candidates = [
            ...
            self.project_root / "credentials" / "token.json",
            self.runtime_data_dir.parent / "token.json",
        ]
        token_path = next((path for path in token_candidates if path.exists()), token_candidates[0])
        ...
        gmail_tokens = {
            "personal": self.credentials_dir / "token_gmail_personal.json",
            "kohara": self.credentials_dir / "token_gmail.json",
        }
...
具体的で行動につながる内容にしてください。"""
    ...
    async def _run_weekly_content_suggestions(self):
        ...
        news_log = self.system_dir / "ai_news.log"
        news_content = ""
        if news_log.exists():
            try:
...
受講生にとって今すぐ価値がある内容にしてください。"""
    ...
    async def _check_follow_up_suggestions(self, send_line_notify):
        ...
        contact_state_path = self.runtime_data_dir / "contact_state.json"
        profiles_path = self.master_dir / "people" / "profiles.json"
        if not contact_state_path.exists() or not profiles_path.exists():
...
===RESULT_END==="""
    ...
    async def _run_cdp_sync(self):
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                if message:
                    from .notifier import send_line_notify
                    if _is_cdp_success_message(message):
                        now = datetime.now(_SCHEDULER_TIMEZONE)
                        if _should_send_cdp_success_notification(now):
                            if send_line_notify(message):
                                _mark_cdp_success_notification_sent(now)
                        else:
                            logger.info("CDP success notification skipped (already sent today)")
                    else:
                        send_line_notify(message)
            else:
    ...
    async def _run_unique_email_sheet_sync(self):
        """2時間ごと: UUメールアドレス管理シートを再生成する。"""
        result = await self._execute_tool(
            "unique_email_sheet_sync",
            tools.unique_email_sheet_sync,
        )
        if result.success:
            logger.info(f"Unique email sheet sync completed: {(result.output or '')[-300:]}")
            return
        ...
        send_line_notify(
            _format_line_message(
                ...
                links=[
                    (
                        "UUメール / メール集計サマリー",
                        _build_sheet_url(_UNIQUE_EMAIL_SHEET_ID, _UNIQUE_EMAIL_SUMMARY_TAB),
                    ),
                    (
                        "UUメール / データソース管理",
                        _build_sheet_url(_UNIQUE_EMAIL_SHEET_ID, _UNIQUE_EMAIL_SOURCE_TAB),
                    ),
                ],
            )
        )
    ...
    async def _run_teleapo_sync(self):
        ...
        script = str(Path(__file__).resolve().parent.parent.parent / "teleapo_sync.py")
        try:
            proc = subprocess.run(
                ["python3", script, "sync"],
                capture_output=True,
            ...
            output = proc.stdout if proc.stdout else ""
            if proc.returncode == 0:
                logger.info(f"Teleapo sync completed: {output[-500:]}")
                self._record_schedule_success("teleapo_sync")
            else:
                error = proc.stderr[-300:] if proc.stderr else output[-300:]
                logger.warning(f"Teleapo sync failed: {error}")
                from .notifier import send_line_notify
                ...
                send_line_notify(
                    _format_line_message(
                        ...
                        summary_lines=[error[:200] or "標準エラーの取得に失敗しました"],
                        action_lines=[
                            "AIテレアポ管理シートの `架電一覧` を開いて、同期されているか確認してください"
                        ],
                        links=[
                            (
                                "AIテレアポ管理 / 架電一覧",
                                _build_sheet_url(_TELEAPO_SHEET_ID, _TELEAPO_TARGET_TAB),
                            )
                        ],
                    )
                )
        except subprocess.TimeoutExpired:
            logger.warning("Teleapo sync timed out after 15 minutes")
            from .notifier import send_line_notify

            send_line_notify(
                _format_line_message(
                    "AIテレアポ一覧の同期がタイムアウトしました",
                    summary_lines=["15分経っても完了しませんでした"],
                    action_lines=[
                        "AIテレアポ管理シートの `架電一覧` を開いて、同期途中で止まっていないか確認してください"
                    ],
                    links=[
                        (
                            "AIテレアポ管理 / 架電一覧",
            ...
            if os.path.exists(feedback_path):
                ...
                if len(corrections) >= 3:
                    ...
                    for i, c in enumerate(corrections[-20:], 1):
                        ...
                        change_labels = c.get("change_labels", [])
                        if change_labels:
                            line += f" 変化:{'/'.join(change_labels)}"
                        context_preview = c.get("context_preview", "")
                        if context_preview:
                            line += f" 文脈:{context_preview[:80]}"
            ...
            if os.path.exists(feedback_path):
                ...
                for person_name_fb, person_fbs in person_corrections.items():
                    ...
                    for c in corrections_for_person[-10:]:
                        ...
                        fb_lines_person.append(line)
                    fb_text_person = "\n".join(fb_lines_person)
                    # group_insightsも参照
...
{{
  ...
  "style_note": "この人への返信スタイルを1文で"
    ...
    async def _reply_as_secretary(self, user_text: str, send_fn):
        ...
        identity_context = ""
        identity_path = self.master_dir / "self_clone" / "kohara" / "IDENTITY.md"
        try:
    ...
    async def _run_secretary_proactive_work(self):
        ...
        state_dir = Path(
            self.config.get("paths", {}).get(
                "db_path",
                str(self.system_dir / "mac_mini" / "agent_orchestrator" / "agent.db"),
            )
        ).expanduser().parent
...
"""
    ...
    def _record_secretary_action(self, cycle: int, result: str):
        ...
        log_path = self.master_dir / "learning" / "secretary_action_log.json"
...
- 請求書の日付・件名・金額を必ず目視確認してからPDF化すること"""
    ...
    async def _run_meeting_report(self):
        """経営会議資料を自動作成: Sheets 正本更新→画像生成→Google Docs 挿入→LINE通知"""
        from .notifier import send_line_notify
        ...
        try:
            _ensure_system_path()
            from mac_mini.tools.meeting_report_pipeline import build_meeting_report

            result = build_meeting_report(meeting_date=today)
            ...
            if sheet_url:
                summary_lines.extend(["", f"・正本シート\n{sheet_url}"])
            summary_lines.extend(
                [
                    "",
                    f"・総評: {score}/5 — {comment}",
                    f"・月次: 着金売上 {int(round(monthly['revenue_actual'] / 10000)):,}万円 / 集客数 {int(round(monthly['lead_actual'])):,}人",
                    f"・週次: 集客数 {weekly['lead_change_pct'] * 100:+.1f}% / CPA {weekly['cpa_change_pct'] * 100:+.1f}%",
                ]
            )
            send_line_notify("\n".join(summary_lines))
            self.memory.set_state("last_success_meeting_report", datetime.now().isoformat())
            logger.info("経営会議資料: 完了")
        except Exception as e:
            error = str(e)
            send_line_notify(
                "経営会議資料の自動作成が失敗しました。\n"
            ...
            logger.exception(f"経営会議資料: 失敗 - {error}")

...
def membership_collection_sheet_sync(dry_run: bool = False) -> ToolResult:
    """会員データ（収集）を更新する"""
    args = []
    if dry_run:
        args.append("--dry-run")
    return _run_script(
        os.path.join(SYSTEM_DIR, "membership_collection_sheet_setup.py"),
        args,
        timeout=1800,
    )


def membership_metrics_sheet_sync(dry_run: bool = False) -> ToolResult:
    """会員データ（加工）を更新する"""
    args = []
    if dry_run:
        args.append("--dry-run")
    return _run_script(
        os.path.join(SYSTEM_DIR, "membership_metrics_sheet_sync.py"),
        args,
        timeout=1800,
    )


def skillplus_autogenerated_metrics_sheet_sync(dry_run: bool = False) -> ToolResult:
    """スキルプラス受講生データ（加工）の自動生成タブを更新する"""
    args = []
    if dry_run:
        args.append("--dry-run")
    return _run_script(
        os.path.join(SYSTEM_DIR, "skillplus_autogenerated_metrics_sheet_sync.py"),
        args,
        timeout=1800,
    )
...
TOOL_REGISTRY = {
    ...
    "membership_metrics_sheet_sync": {"fn": membership_metrics_sheet_sync, "description": "会員データ（加工）を更新"},
    "skillplus_autogenerated_metrics_sheet_sync": {"fn": skillplus_autogenerated_metrics_sheet_sync, "description": "スキルプラス受講生データ（加工）の自動生成タブを更新"},

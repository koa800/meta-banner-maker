...
- [ ] `local_agent.py` をMac miniに移行する（Personal MacがスリープするとLINE返信が止まる）
- [ ] Mac mini Orchestratorが実際に稼働しているか確認・起動
- [ ] 環境変数・OAuthトークンをMac miniに移行（transfer_tokens.sh実行）
...
### 日次報告の自動化
- [ ] **毎朝8:30 日次サマリーをLINE送信**: 今日の期限タスク + 期限超過ゴール一覧 + Addness上の自分担当タスク
  - *根拠*: Addness上に期限超過が多数。毎日の見落とし防止

### Q&A自動回答（広告コース）
- [ ] **qa_monitor.pyを本番稼働させる**: スタンダードコース・広告コースの質問を自動検知→回答案生成→承認→送信
  - *根拠*: 5-10分/日の削減 + 回答漏れゼロへ。学習体験に直結

### メール自動処理
- [ ] **重要メール検知→返信案をLINE通知**: GmailでCV・契約・メディア依頼などを検知したら即通知
  - *根拠*: 3分/日の作業 + 見落とし防止

### 期限管理エージェント
- [ ] **毎週月曜9:00 期限超過・今週期限ゴールをLINE通知**: Addnessから自動取得して整理

...
> 最終更新: 2026-02-21 (P3/P4: バナー生成・アフィリエイト提案・競合分析・週次ボトルネック)
...
- [ ] **アフィリエイト用コンテンツ自動整備提案**: 成約率が低いアフィリエイターへの支援コンテンツを自動生成（実データなしで根拠薄いため廃止）
...
- [ ] **返信スタイル自動学習**: 甲原さんが実際に送った返信（修正テキスト・承認済み返信）とグループメッセージを蓄積し、定期的にパターンを分析。IDENTITY.mdやcomm_profileの表現ルール・口調を自動更新する
  - *根拠*: 現在はreply_feedback.jsonで承認/修正を記録しているが、分析→ルール反映は手動。使うほど精度が上がる仕組みにする
  - *ステップ1*: 修正返信・承認返信から「使う表現/使わない表現」パターンを週次で抽出
  - *ステップ2*: 抽出結果をIDENTITY.md・comm_profileに自動反映（差分提案→承認で適用）
...
- [ ] **L-step開封率・反応率週次サマリー**
- [ ] **競合比較エージェント**: スキルプラスvs競合3-5社の価格・訴求の差分を月1でまとめる（実データなしで根拠薄いため廃止）

---

## 💡 積み残しアイデア（未分類）

- ~~Mac miniが「今週のボトルネック」をAddnessゴールから分析して提案する~~ → `_notify_weekly_bottleneck` で実装済み
- LINEから「次に何すべき？」と聞いたらAddnessゴール+メール+カレンダーを総合して答える
- **AI動線・SNS動線のファネル知識をSkills化**: シート上のSensors/AIパネルのファネル構造をナレッジとして取り込み、返信時に動線の文脈を理解できるようにする
...
- ~~**weekly_affiliate_ideas**~~: 実データなしで根拠薄いため廃止（2026-02-21）
...
- ~~**monthly_competitor_analysis**~~: 実データなしで根拠薄いため廃止（2026-02-21）

...
- UTAGE
  ...
  - exact save は modal の保存ボタン `#button-name`
  - `ZZ_TEST_UTAGE_video_probe_UPDATED` への rename と rollback まで live で確認済み
  - したがって `動画管理 > 名称変更` は row 名変化を一次判定に置く
...
- UTAGE
  ...
  - `動画管理 > 名称変更` は `#button-name` で small change -> rollback 済み
...
- Mailchimp
  ...
  - ただし `mailchimp_tfa_code_helper.py --max-days 1 --max-age-minutes 180 --wait-seconds 30 --poll-interval-seconds 5` は `found=false`

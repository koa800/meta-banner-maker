...
- `【アドネス株式会社】マスタデータ / 決済商品変換マスタ` を新設済み
  ...
  - `タスクマさん / タスクデビル` は終了した新規事業扱いとして `決済商品変換マスタ` から除外する。`決済データ（収集）` の raw は検証用途のため残す
  - `要確認 / 商品マスタ未登録 / 不明` の行は、判断が必要なセル `E:N` を色付きにして残す
  - 最新件数は `変換済み 222 / 要確認 1 / 商品マスタ未登録 1 / 不明 6`
...
- INVOY の generic 名
  ...
  - よって generic な `お支払い代金` 系は自動変換せず `要確認` のまま残す
- 現時点の結論
  - 自動で埋めてよいのは `明示エイリアス` と `商品名が明示されているキーワード一致` まで
  - `商品名空欄` と generic INVOY は、加工時も `不明` または `要確認` として扱う前提が必要

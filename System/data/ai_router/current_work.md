...
- `【アドネス株式会社】決済データ（加工）` を新設済み
  ...
  - `商品名空欄` のうち `日本プラム / きらぼし銀行 / CBS / 京都信販 / CREDIX` の成功売上は、`スキルプラス受講生データ（元）` の一次データで確認でき、かつ `他事業の成約リスト` と競合しないものだけを `プラン未特定売上` として着金売上へ含める
  ...
  - 2026-03-20 13:55 時点の state:
    - 累計着金売上 `4,018,108,426`
    - 累計返金額 `798,907,709`
    - 累計純着金売上 `3,219,150,717`
    - 累計解約請求額 `75,218,484`
    - 完全不明金額 `1,840,315,132`
    - 完全不明件数 `19,781`
    - 補完返金件数 `4,299`

...
def build_summary_rows(booking_status: Dict[str, str]) -> List[List[str]]:
    ...
    visible_rows = [
        ...
        ["個別予約数", '=IF(OR($B$2="",$B$3=""),"",IF(COUNTIFS(\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3,\'日別数値\'!D:D,"<>")=0,"",SUMIFS(\'日別数値\'!D:D,\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)))', "【アドネス株式会社】個別面談データ / 日別個別予約数", booking_state, "現行は botログ。継続体制では Slack #個別予約通知 から作る 個別予約通知ログへ移行する"],
        ...
        ["会員数", '=IF(OR($B$2="",$B$3=""),"",IFERROR(INDEX(FILTER(\'日別数値\'!L:L,\'日別数値\'!A:A>=$B$2,\'日別数値\'!A:A<=$B$3,\'日別数値\'!L:L<>""),ROWS(FILTER(\'日別数値\'!L:L,\'日別数値\'!A:A>=$B$2,\'日別数値\'!A:A<=$B$3,\'日別数値\'!L:L<>""))),""))', "【アドネス株式会社】顧客データ（複数イベント） / 顧客マスタ", "確認待ち", "入会日・クーリングオフ日・中途解約日から日次残高を作る必要あり"],
        ["中途解約数", '=IF(OR($B$2="",$B$3=""),"",IF(COUNTIFS(\'日別数値\'!M:M,"<>",\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)=0,"",SUMIFS(\'日別数値\'!M:M,\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)))', "【アドネス株式会社】顧客データ（複数イベント） / 顧客マスタ", "確認待ち", "顧客マスタの中途解約日を日別集計へ落とす"],
        ["クーリングオフ数", '=IF(OR($B$2="",$B$3=""),"",IF(COUNTIFS(\'日別数値\'!N:N,"<>",\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)=0,"",SUMIFS(\'日別数値\'!N:N,\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)))', "【アドネス株式会社】顧客データ（複数イベント） / 顧客マスタ", "確認待ち", "顧客マスタのクーリングオフ日を日別集計へ落とす"],
    ]

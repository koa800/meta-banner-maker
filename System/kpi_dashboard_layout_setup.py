...
def build_definition_rows() -> List[List[str]]:
    return [
        ...
        ["個別予約CPO", "広告費 / 個別予約数"],
        ["ROAS", "着金売上 / 広告費"],
        ["会員数", "契約書を巻いたユーザー数から、クーリングオフ数を引いたユーザー数"],
        ["中途解約数", "お客様窓口にて中途解約が確定したユーザー数"],
        ["クーリングオフ数", "契約書を巻いたユーザー数のうち、7日以内に解約が確定したユーザー数"],
    ]
...
def build_summary_rows(booking_status: Dict[str, str]) -> List[List[str]]:
    ...
    visible_rows = [
        ...
        ["会員数", '=IF(OR($B$2="",$B$3=""),"",IFERROR(INDEX(FILTER(\'日別数値\'!L:L,\'日別数値\'!A:A>=$B$2,\'日別数値\'!A:A<=$B$3,\'日別数値\'!L:L<>""),ROWS(FILTER(\'日別数値\'!L:L,\'日別数値\'!A:A>=$B$2,\'日別数値\'!A:A<=$B$3,\'日別数値\'!L:L<>""))),""))', "【アドネス株式会社】顧客データ_複数イベント（統合） / 顧客マスタ", "確認待ち", "入会日・クーリングオフ日・中途解約日から日次残高を作る必要あり"],
        ["中途解約数", '=IF(OR($B$2="",$B$3=""),"",IF(COUNTIFS(\'日別数値\'!M:M,"<>",\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)=0,"",SUMIFS(\'日別数値\'!M:M,\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)))', "【アドネス株式会社】顧客データ_複数イベント（統合） / 顧客マスタ", "確認待ち", "顧客マスタの中途解約日を日別集計へ落とす"],
        ["クーリングオフ数", '=IF(OR($B$2="",$B$3=""),"",IF(COUNTIFS(\'日別数値\'!N:N,"<>",\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)=0,"",SUMIFS(\'日別数値\'!N:N,\'日別数値\'!A:A,">="&$B$2,\'日別数値\'!A:A,"<="&$B$3)))', "【アドネス株式会社】顧客データ_複数イベント（統合） / 顧客マスタ", "確認待ち", "顧客マスタのクーリングオフ日を日別集計へ落とす"],
    ]
...
def build_data_source_rows(status: Dict[str, str], daily_row_count: int, booking_status: Dict[str, str], booking_daily_row_count: int) -> List[List[str]]:
    ...
    rows = [
        ...
        ["会員数", "会員", "統合データ候補", "1", '=HYPERLINK("https://docs.google.com/spreadsheets/d/1qjU279OVD0i4h2AdQzkYIsZCfA1BeiUKLHNg7i2a2fk/edit","【アドネス株式会社】顧客データ_複数イベント（統合）")', "顧客マスタ", "入会日 / クーリングオフ日 / 中途解約日", "日次残高", "会員イベントの定義確定後", "確認待ち", "", "", "", "日別残高シートが未作成"],
        ["中途解約数", "会員", "統合データ候補", "1", '=HYPERLINK("https://docs.google.com/spreadsheets/d/1qjU279OVD0i4h2AdQzkYIsZCfA1BeiUKLHNg7i2a2fk/edit","【アドネス株式会社】顧客データ_複数イベント（統合）")', "顧客マスタ", "中途解約日", "日別件数", "中途解約日の集計ルール確定後", "確認待ち", "", "", "", "顧客マスタを日別集計へ落とす"],
        ["クーリングオフ数", "会員", "統合データ候補", "1", '=HYPERLINK("https://docs.google.com/spreadsheets/d/1qjU279OVD0i4h2AdQzkYIsZCfA1BeiUKLHNg7i2a2fk/edit","【アドネス株式会社】顧客データ_複数イベント（統合）")', "顧客マスタ", "クーリングオフ日", "日別件数", "クーリングオフ日の集計ルール確定後", "確認待ち", "", "", "", "顧客マスタを日別集計へ落とす"],
    ]
    while len(rows) < 30:
        rows.append([""] * 14)
    return rows

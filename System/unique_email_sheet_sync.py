...
母集団:
- 【アドネス株式会社】顧客データ_メールアドレスのみ（統合）: 未転換リード
- 【アドネス株式会社】顧客データ_複数イベント（統合）: 顧客マスタ

登録日補完:
- 【アドネス株式会社】集客データ_メールアドレス（収集）: UTAGE元データ
- 過去のUTAGE登録者CSV: 過去分の補完
- 【アドネス株式会社】顧客データ_メールアドレスのみ（統合）: 旧メール集客データ
...
def build_source_management_rows() -> List[List[str]]:
    return [
        ...
        [
            ...
            "",
            "【アドネス株式会社】集客データ_メールアドレス（収集）に日付が無いメールだけを補完する履歴",
        ],
        [
            "初回整備のみ",
            "顧客データ（メールアドレスのみ）",
...
def build_rule_rows() -> List[List[str]]:
    return [
        ...
        ["登録日", "【アドネス株式会社】集客データ_メールアドレス（収集）の各流入経路タブ A列 を正とし、そこに日付が無いメールだけを過去CSVとメール集客データで補完する", "登録日の正本を1つに固定するため"],
...
def collect_registration_dates(gc) -> Tuple[Dict[str, str], Dict[str, int]]:
    ...
    # 「【アドネス株式会社】集客データ_メールアドレス（収集）」の各流入経路タブ A列を

...
def build_rule_rows() -> List[List[str]]:
    return [
        ...
        ["追加するもの", "個別予約通知ログを 1イベント1行で持つ", "過去の個別予約データと Slack #個別予約通知 を同じ正本へ統合する"],
        ["個別予約数", "現行は【アドネス】顧客管理シート / 個別予約集計botログ を使う", "日付あり and キャンセル=FALSE の行だけを数える"],
        ["個別予約通知ログ", "過去の個別予約データを引き継ぎつつ、Lステップ の通知機能で ★【個別予約完了】★ の付与を1イベント1行で溜める", "まずは Slack の専用チャンネルに集約し、難しい時だけ Chatwork を使う"],

...
LINEで読める形式で、合計600文字以内に収めてください。"""
        ...
        # ===== フィードバック学習状況確認タスク =====
        if function_name == "feedback_status":
            try:
                examples = load_feedback_examples()
                notes = [e for e in examples if e.get("type") == "note"]
                corrections = [e for e in examples if e.get("type") == "correction"]
                # 最新の修正例を取得
                recent = sorted(corrections, key=lambda e: e.get("timestamp", ""), reverse=True)[:3]
                parts = [
                    "🎓 学習状況",
                    "━━━━━━━━━━━━",
                    f"スタイルノート: {len(notes)}件",
                    f"修正例: {len(corrections)}件",
                ]
                if recent:
                    parts.append("\n【直近の修正例】")
                    for e in recent:
                        sname = e.get("sender_name", "不明")
                        actual = e.get("actual_sent", "")[:30]
                        parts.append(f"  {sname}: 「{actual}...」")
                if notes:
                    parts.append("\n【スタイルノート（直近2件）】")
                    for n in notes[-2:]:
                        parts.append(f"  ・{n.get('note', '')[:40]}")
                parts.append("━━━━━━━━━━━━")
                return True, "\n".join(parts)
            except Exception as e:
                return False, f"学習状況取得エラー: {str(e)}"

        # ===== フォローアップ確認タスク =====
        if function_name == "followup_check":
            contact_state_path = _AGENT_DIR / "contact_state.json"
            if not contact_state_path.exists():
                return False, "接触記録がまだありません\n（返信案を承認・送信すると記録が蓄積されます）"
            try:
                contact_state = json.loads(contact_state_path.read_text(encoding="utf-8"))
                profiles = {}
                if PEOPLE_PROFILES_JSON.exists():
                    raw = json.loads(PEOPLE_PROFILES_JSON.read_text(encoding="utf-8"))
                ...
                now = _dt.now()
                THRESHOLDS = {"上司": 30, "横（並列）": 21, "直下メンバー": 14, "メンバー": 14}
                overdue = []
                recent_contacts = []
                for name, last_str in sorted(contact_state.items(), key=lambda x: x[1], reverse=True):
                    try:
                        last = _dt.fromisoformat(last_str)
                        days = (now - last).days
                        cat = profiles.get(name, "")
                        thresh = THRESHOLDS.get(cat, 0)
                        if thresh and days >= thresh:
                            overdue.append((days, name, cat))
                        elif days <= 7:
                            recent_contacts.append((days, name, cat))
                    except Exception:
                        pass

                parts = ["💬 フォローアップ状況", "━━━━━━━━━━━━"]
                if overdue:
                    parts.append(f"⚠️ 要フォローアップ ({len(overdue)}名):")
                    for days, name, cat in sorted(overdue, reverse=True)[:5]:
                        cat_str = f"({cat})" if cat else ""
                        parts.append(f"  {name}{cat_str}: {days}日未連絡")
                if recent_contacts:
                    parts.append(f"\n✅ 直近7日に連絡済み:")
                    for days, name, cat in sorted(recent_contacts)[:5]:
                        parts.append(f"  {name}: {days}日前")
                if not overdue and not recent_contacts:
                    parts.append("記録された連絡がありません")
                parts.append("━━━━━━━━━━━━")
                return True, "\n".join(parts)
            except Exception as e:
                return False, f"フォローアップ確認エラー: {str(e)}"

...
成長レベル: Lv.1（従業員型）— なぜここから→ドメイン理解が先。理解が浅いまま動くと的外れなアクションで信頼を失う。報告なら何も壊れない

**Lv.1 で鍛える2つのスキル:**
1. **意図汲み取り力**: 甲原さんの「何を」だけでなく「なぜ」を考える。自信あればそのまま動く。なければ「こう理解しましたが合っていますか？」と確認してから動く
2. **コード修正力**: 現在のゴール「システム構築」に直結。バグ修正・機能追加を正確に。テストを通し、副作用を出さない

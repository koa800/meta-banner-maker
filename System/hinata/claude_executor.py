...
## 動作モード: {"報告" if mode == "report" else "提案" if mode == "propose" else "実行"}

{"### 報告モード（現在）" if mode == "report" else ""}
{"信頼を積み上げるフェーズです。まずは正確な状況把握と報告に徹してください。" if mode == "report" else ""}

### ステップ1: Addnessでゴールとアクションを確認
1. `tabs_context_mcp` でタブを確認
2. ゴールページに遷移: `navigate` で `{goal_url}` へ
3. `read_page` でゴールの内容、現在のアクション、コメントを確認
4. 各アクションの進捗状況（完了/未完了/期限）を整理

{"### ステップ2: 状況を報告" if mode == "report" else "### ステップ2: アクションを実行"}
{'''以下の形式で簡潔に報告してください:
- ゴールの現在の状態
- 各アクションの進捗（完了/未完了/期限）
- 甲原さんからの新しいコメントがあればその内容
- 次にやるべきことの提案（あれば）

**アクションの実行は行わない。** 報告と提案のみ。''' if mode == "report" else '''- AIと相談で決まったアクションを実行する
- Addness上の操作（アクション完了・期限設定・新規作成等）もMCPツールで行う
- Web調査が必要なら `tabs_create_mcp` で別タブを開いて調査
- 甲原さんへの確認が必要なら、Addnessでコメント（@甲原海人をつける）'''}

### {"ステップ3" if mode != "report" else "ステップ3（任意）"}: ナレッジを蓄積

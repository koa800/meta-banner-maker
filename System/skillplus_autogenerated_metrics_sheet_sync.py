...
from skillplus_student_metrics_sheet_sync import (
    ...
    style_student_tab,
    style_table,
    merge_student_rows_with_activity_rows,
...
SOURCE_MANAGEMENT_TAB_NAME = "自動生成_データソース管理"
RULE_TAB_NAME = "自動生成_データ追加ルール"
STUDENT_TAB_NAMES = {
    "受講生一覧",
    "受講生サマリー",
    "データソース管理",
...
PROTECTION_PREFIX = "スキルプラス受講生データ（加工）_自動生成"
...
def ensure_tabs(spreadsheet):
    ...
    target_names = set(TAB_SPECS.keys()) | STUDENT_TAB_NAMES
...
def build_auto_aggregates(source_ss) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    for tab_name, default_plan in ROSTER_TAB_SPECS.items():
        ws = run_read_with_retry(
            f"{tab_name} タブの取得",
            lambda tab_name=tab_name: source_ss.worksheet(tab_name),
        )
    ...
    event_ws = run_read_with_retry(
        f"{EVENT_TAB_NAME} タブの取得",
        lambda: source_ss.worksheet(EVENT_TAB_NAME),
    )
...
def sync_skillplus_autogenerated_metrics_sheet(dry_run: bool = False, gc=None) -> dict:
    with FileLock(LOCK_PATH):
        ...
        if "受講生一覧" in tabs:
            student_rows = get_all_values_with_retry(tabs["受講生一覧"])
            if student_rows and len(student_rows[0]) >= len(STUDENT_TAB_WIDTHS):
                merged_student_rows, _ = merge_student_rows_with_activity_rows(student_rows, auto_rows)
                write_rows(target, tabs["受講生一覧"], merged_student_rows)
                style_student_tab(target, tabs["受講生一覧"])

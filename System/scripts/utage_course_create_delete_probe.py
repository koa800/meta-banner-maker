#!/usr/bin/env python3
"""UTAGE の exploratory course を create -> delete で検証する。"""
...
def build_name() -> str:
    return f"ZZ_TEST_{datetime.now().strftime('%Y%m%d_%H%M%S')}_UTAGE_course_probe"


def _find_raw_target_id() -> str:
    target = (
        find_target(url_contains=f"school.addness.co.jp/site/{SITE_ID}/course")
...
def _raw_fill_course_form(target_id: str, name: str) -> bool:
    expression = f"""
(() => {{
  ...
  const nameInput = byLabel('コース名') || document.querySelector('input[name="name"], input');
  if (!nameInput) return false;
  nameInput.focus();
  setValue(nameInput, {json.dumps(name, ensure_ascii=False)});

  const manageInput = byLabel('管理名称');
  if (manageInput) {{
    manageInput.focus();
    setValue(manageInput, {json.dumps(name + "_mg", ensure_ascii=False)});
  }}

  const urlInput = byLabel('リンク先URL');
  if (urlInput) {{
    urlInput.focus();
    setValue(urlInput, 'https://example.com/utage-course-probe');
  }}
  return true;
}})()
"""
...
def run_probe_raw() -> dict[str, Any]:
    ...
    time.sleep(2.5)
    if not _raw_fill_course_form(target_id, name):
        raise RuntimeError("raw fallback で コース作成フォーム を入力できませんでした")
...
async def run_probe() -> dict[str, Any]:
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.get_by_label("リンク先URL").fill("https://example.com/utage-course-probe")

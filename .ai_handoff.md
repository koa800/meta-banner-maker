...
- 2026-03-17 の current では `Assets > Zaps > Create` からの新規 draft 作成も live で再確認した
  ...
  - つまり current では `Zap details > Folder` の即時反映は不安定でも、assets list 起点で create しつつ `Location` を別途検証する前提で扱う
  - 2026-03-17 以降の正本は、Zapier 新規 relay の主入口を `Assets > Zaps > Create` に固定する
  ...
  - `python3 System/scripts/zapier_create_delete_probe.py --with-action`
    - `created_edit_path = /webintent/edit-zap/354627766`
    - `after_delete_count = 10`
  - `python3 System/scripts/zapier_second_action_probe.py`
    - `created_edit_path = /webintent/edit-zap/354627984`
    - second action 後の step node は `1. Catch Hook / 2. Action / 3. POST / 4. Action`
    - cleanup は `/webintent/edit-zap/354627984` で `after_count=0`
...
- 2026-03-17 の current では `バンドルコース一覧 > 編集` も raw probe で `1変更 -> rollback -> delete` を再確認した
  - probe: `python3 System/scripts/utage_bundle_edit_rollback_probe.py`
...
- current で一番確実なのは、`MailChimp認証` グループの実画面で最新コードを確認して入力する flow
...
- UTAGE
  ...
  - `動画管理 > 名称変更` は visible form action `/video/name/update` が見えていても、raw `form.submit()` では row 名が変わらない current 挙動を確認
  - 同じ modal の `保存` ボタン `#button-name` を押すと row 名は intended に変わり、rollback も同じ手順で戻せることを確認
  - したがって `動画管理 > 名称変更` は row 名変化を一次判定に置き、exact な save は `#button-name` click とする
...
- Zapier
  ...
  - `Create` を開くと `New Zap | Zapier` の editor へ入る
  - `Create` を開いただけでは assets 一覧に `Untitled Zap` は persisted しない current 挙動
  ...
  - cleanup は `--edit-path` 指定で target draft を直接消せる
  - 直近の exact cleanup は `edit-zap/354619948` で `before_count=1 -> after_count=0` を確認済み
  - persisted 済み draft は `assets 一覧 row action Delete -> cleanup`
  - `python3 System/scripts/zapier_create_delete_probe.py --with-action`
    - `Create -> Trigger > Webhooks by Zapier > Catch Hook -> Action > Mailchimp > Add/Update Subscriber -> Test -> Delete Zap` を live 完了
  - `python3 System/scripts/zapier_create_delete_probe.py --with-action --action-app webhook-post`
    - `Create -> Trigger > Webhooks by Zapier > Catch Hook -> Action > Webhooks by Zapier > POST -> Test -> Delete Zap` を live 完了
...
- Zapier live probe
  ...
  - `Create -> Trigger -> Webhooks by Zapier -> Catch Hook -> persisted draft -> assets cleanup` を current UI で確認
...
- UTAGE
  ...
  - `動画管理 > 名称変更` は save/rollback 済みなので、残差は representative の反復本数
...
- Mailchimp
  ...
  - 2026-03-17 の probe では TFA 画面の `Send code` 自体は click 成功
  - ただし `mailchimp_tfa_code_helper.py --max-days 1 ...` は `found=false`
  - current blocker は `Send code` ではなく、today の認証コードが `group-log API` で取れないこと
- Zapier
  - `Create -> trigger 1つ選択 -> Test -> Delete Zap` の live 実作成
...
- 残差は representative family を増やすこと
  ...
  - `Create` 後の persisted 判定は timing と assets 一覧文脈の影響を受ける
...
- Zapier
  - `Catch Hook -> Mailchimp Add/Update Subscriber`
    の最小 relay を `Create -> trigger 1つ選択 -> Test -> Delete Zap`
- short.io

...
1. **自動実行で確認**: 翌朝 08:40 の `daily_report_input` で成功すればOK。失敗時は LINE / Slack に通知が届く。
...
- **対象日**: 前日分
- **タイムアウト**: 10分（ブラウザ操作があるため長め）
- **リトライ**: 失敗時 60秒後に自動リトライ（最大2回）
- **失敗時**: LINE + Slack の両方に通知。手動で「日報入れて」と指示すれば再実行可能
- **完了検証**: 09:20 に `daily_report_verify` がシートの実データを自動確認。未入力があれば LINE+Slack で通知

### プリフライトチェック（自動実行前に確認される項目）

| チェック項目 | 失敗時の動作 |
|---|---|
| Claude Code CLI 存在 | LINE+Slack で通知、タスク中止 |
| `~/.claude-secretary` 存在 | LINE+Slack で通知、タスク中止 |
...
| ページ | URL |
|---|---|
| 日別データ（集客数・個別予約数・着金売上） | `https://lookerstudio.google.com/u/2/reporting/f3d08756-9297-4d34-b6ea-ea22780eb4d2/page/p_evmsc9twzd` |
| スキルプラス会員数（会員数・解約数） | `https://lookerstudio.google.com/u/2/reporting/f3d08756-9297-4d34-b6ea-ea22780eb4d2/page/p_dfv0688m0d` |

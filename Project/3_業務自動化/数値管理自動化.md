...
| 最終更新 | 2026年3月9日（Meta広告CR raw の Looker取得と失敗パターン集計を追加） |
...
Looker Studio
    ...
    ▼  Orchestrator 11:30 looker_csv_download
       ...
       ・07:00 looker_session_keepalive（CDPでセッション維持）
       ・ログイン切れ時: 自動ログイン（koa800sea.nifs→kohara.kaito）
...
## 補助パイプライン: Meta広告CR raw

KPI 集計とは別に、Meta広告の CR 一覧 raw も Looker から定期取得する。目的は日次KPIの更新ではなく、`失敗CRの型抽出` と `Master の知識化` に使うこと。

```text
Looker Studio（Meta広告CR一覧）
    │
    ▼  Orchestrator 11:45 meta_cr_dashboard_download
~/Desktop/Looker Studio CSV/meta_cr_dashboard/
  {YYYY-MM-DD}_Meta広告CR一覧_表.csv
    │
    ▼  System/meta_cr_dashboard_sync.py analyze
System/data/meta_cr_dashboard/
  latest_normalized.csv
...
- raw は `何が外れたか` を知るための入力元
- 原因断定はしない。LP・面談定性・下流売上と重ねる
- 最初に `高フックでも外れる型` と `低CTRで止まる型` を分ける
...
| `System/meta_cr_dashboard_sync.py` | Meta広告CR raw の正規化と失敗パターン集計 |

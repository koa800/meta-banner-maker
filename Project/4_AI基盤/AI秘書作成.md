...
| プロジェクト名 | AI秘書作成 |
| 開始日 | 2026年2月18日 |
| 最終更新 | 2026年3月17日（LINE本物メンション送信とグループ member 解決 API まで実装反映） |
| ステータス | 🚀 継続開発中 |
...
原則は `甲原さんの次の行動が変わる情報だけ通知する`。

- `即時通知`: 承認・認証・判断・期限・顧客影響・自動復旧不可
- `まとめ通知`: 今日見る価値はあるが即時性はない定常サマリ
- `非通知`: 内部成功、自己回復成功、keepalive、cache/sync/deploy の正常完了

初期実装として、`agent_orchestrator/notifier.py` に policy 読み込みと `notify_event()` / `flush_digest_events()` を追加した。
...
- `21:00 稼働報告`
- `Render デプロイ完了`
- `csv_sheet_sync.py` の成功通知

は即時送信を止めた。

`メール返信待ち` は digest queue に入れ、朝夕のまとめから出せる土台に寄せた。
...
40. **プロンプト最適化**: Claude Haiku で日本語の曖昧な指示→最適な英語プロンプトに変換（スタイル・照明・構図・色彩・品質を含む）
41. **参照画像サポート**: LINEで画像を送信→5分以内に「この感じで〇〇作って」と指示→Claude Vision で画像分析→スタイル・構図を取り込んだプロンプト生成
42. **修正ループ**: 生成後30分以内の「もう少し明るくして」「もっとシンプルに」等のフォローアップで、前回の生成コンテキスト（プロンプト・設定）を自動引き継ぎ
43. **多段フォールバック**: Gemini API → Pollinations.ai（無料API）のフォールバック構成
44. **画像アップロードAPI**: Render `/api/upload_image` で画像受付、`/api/images/<filename>` で配信（HTTPS直リンク）
45. **月間予算制限（¥2,000）**: アプリレベルで `gemini_usage.json` に月間使用量を記録し、2000円（約100枚）で自動停止→Pollinationsにフォールバック。GCP予算アラート（50%/90%/100%でメール通知）も設定済み
...
55. **空き時間判定**: 深夜早朝（22-07時）・定常業務時間帯（日報入力/Looker CSV/日次レポート±15分バッファ）・前回実行20分以内はスキップ。定常業務 > LINE指示 > ゴール進行の優先順位を維持
...
48. **秘書の自己認識**: システムプロンプトに使用ツール・モデル情報（`SELF_AWARENESS_TEXT`）を追加。画像生成=Gemini 2.5 Flash、テキスト=Claude Haiku 4.5等。「分かりません」と答える問題を構造的に解決
...
### OSすり合わせ（秘書→甲原 / 「下から上へ」原則）

| トリガー | 動作 |
|----------|------|
| **毎週金曜20:00（自動）** | 秘書が自分から甲原さんにLINEでOS認識レポートを送信。薄い部分を質問 |

**往復ループ（自動）:**
1. 秘書がレポート＋質問をLINEに送信 → `os_sync_state.json` に `pending` 保存
2. 甲原さんがLINEで回答 → local_agentが自動検知（Claude判定）
3. 新情報を抽出 → 「これをOSに反映していい？」と確認
4. 甲原さんが「更新して」→ `execution_rules.json` + `BRAIN_OS.md` に自動反映 → Render にも自動同期（`/api/sync_execution_rules`）
5. 48時間で自動期限切れ（応答なしなら次回へ持ち越し）

> 「下から上へ」の原則: 秘書側から能動的にすり合わせる。甲原さんが動かなくても回る仕組み。
> Googleカレンダーにも繰り返し予定として登録済み（金曜20:00 / 15分）
...
| 金曜 | 20:00 | `os_sync_session` | 🟡 | 秘書→甲原: 脳のOSすり合わせレポート送信（価値観・判断軸・行動ルールの認識確認＋質問） | ✅ あり |
...
| エンドポイント | メソッド | 説明 |
|-------------|---------|------|
| `/notify` | POST | 秘書グループまたは指定グループに plain text を送信 |
| `/notify/mention` | POST | 指定グループに本物の LINE メンション付きで送信 |
| `/api/group-members` | GET | 指定グループの `user_id / user_name` 一覧取得（`group_id` 必須、`name` で部分一致フィルタ可） |
...
- [x] メモコマンド
- [x] 引用返信（リプライ）検知
- [x] 引用返信通知文面の改善（初回通知と返信案通知を引用返信専用フォーマットに統一）

...
| 開始日 | 2026年2月25日 |
| 最終更新 | 2026年2月25日（Phase 3 実装完了） |
| ステータス | ✅ Phase 1-3 実装完了 |
| 前提 | `Project/4_AI基盤/AI秘書作成.md`（現行システム）の上位互換 |
...
変更:
  ① handler_runner.py に api_call / workflow_endpoint / mcp の汎用呼び出しを実装 ✅
     - api_call: profiles.json の preferred_agent → interface.config に従って外部API呼び出し
       - Perplexity: OpenAI互換API（Web検索）
       - 汎用: endpoint に POST（Lubert, 動画AI, 将来のAPI）
     - workflow_endpoint: Mac Mini Orchestrator 等のワークフローエンドポイントに HTTP POST
     - mcp: MCP プロトコル（将来用プレースホルダー）
  ② tool_registry.json に search / generate_image / generate_video を追加 ✅
     - search: Perplexity API 経由のWeb検索（L1）
     - generate_image: Lubert API 経由の画像生成（L2）
     - generate_video: 動画AI API 経由の動画生成（L2）
  ③ ワークフロー移譲の transfer_status 管理を実装 ✅
     - Coordinator のシステムプロンプトに Phase 1-4 の移譲ルールを追加
     - _load_agent_summary にワークフロー型エージェントの表示を追加
```

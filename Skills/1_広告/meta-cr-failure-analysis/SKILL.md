...
- 下流確認の正本: CDP `顧客マスタ`（`CR名 / 遷移先リンク / 着金売上 / 返金額 / LTV`）
- CR×LP照合の正本: `CR別データ` シート（`CR名 / LP名 / CR×LP / 集客媒体 / 遷移先リンク`）
...
3. 次に `同一アセット比較` を入れる。  
   ...
   - いまの raw では、`運用差` は主に `オーディエンス / 広告ID / 広告セットID` 側で読み、`運用者名` は主分析に使わない  
   - `同じ動画URL/画像 × LP違い` → LP とオファーの期待値差を疑う  
   - `同じ動画URL/画像 × 同LP × 時期違い` → 旬・市場背景・既視感の差を疑う  
   - `同テーマ × 冒頭違い` → 冒頭構成差を疑う  
...
7. `CRコード` で CDP と結線する。  
   - Meta raw の `広告名` と CDP の `CR名` から末尾 `CR\d+` を抜き出して結線する  
   - `CRリンク` は facebook watch URL が多く、主結線キーには使わない  
   - LP別の下流集計は、まず `CR別データ` シートの `遷移先リンク` を優先して URL 単位で使う  
   - `LP1 / LP4` のようなラベルは補助情報で、共有や判断の主表現に使わない  
   - `CR別データ` でも単一LPに解けないときだけ raw 側を補助的に使う  
   - それでも同じCRコードが複数LPにまたがるなら、そのCRは LP別集計から除外する

8. 下流数値で切り分ける。  
   - `CTRとフック率は通るがオプトイン以降で失敗` が多いときは、まず `LP / オファー / 導線 / ターゲット層` を疑う  
   - `クリックされるがCPAが高い` は、クリック後の `オプトイン率` と `ターゲット層の属性` を疑う  
   - この文脈で `CVR` は `オプトイン率` を指す  
   - LPのオプトインCVRだけでなく、`着金売上 / 返金額 / 顧客あたりLTV` まで見て、`オプトインCVRは高いが狙っていない属性が混ざるLP` を見落とさない

9. full raw を取る。認証済みの `CR一覧` タブが開いている状態なら、これで long-range CSV を保存できる。
...
12. 失敗バケットをまたいで `全件` 読みたいときは、`--all-buckets` を必ず付ける。

...
8. 下流数値で切り分ける。  
   ...
   - `CVR` は抽象語なので、この分析では `LP到達に対するオプトイン率` のように母数と行動をセットで書く  
   - LPの `LP到達に対するオプトイン率` だけでなく、`着金売上 / 返金額 / 顧客あたりLTV` まで見て、`オプトイン率は高いが狙っていない属性が混ざるLP` を見落とさない

9. まとめるときは、少なくとも `1-2本` は `Master/前提/判断軸.md` の実務判断テンプレで具体化する。  
   - `市場 / 属性 / 状態 / 問題 / 課題 / 供給 / オファー / 取引成立 / 3つの壁 / 数値 / 仮説 / 判断` を埋める  
   - 保存先は `Master/output/YYYY-MM-DD_Meta広告判断テンプレ適用例.md` を使う  
   - 机上の一般論で終わらせず、`具体情報 + 自分の考え + 次に直す場所` まで残す

10. full raw を取る。認証済みの `CR一覧` タブが開いている状態なら、これで long-range CSV を保存できる。
...
13. 失敗バケットをまたいで `全件` 読みたいときは、`--all-buckets` を必ず付ける。

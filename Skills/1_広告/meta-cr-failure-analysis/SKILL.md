...
- `1年後悔` `林さん` のような家系名そのものを `失敗CR` と呼ばない
...
- 同じ家系でも `勝ちCR` と `失敗CR` は混在する
- 家系名は `比較ラベル` として使い、家系名だけで勝ち負けを断定しない
...
- 家系を読むときは `家系 × 月 × 崩れ方` を並べ、`上流は通るが後ろで失敗 -> 最初で止まる` のような変化を `旬切れ / 既視感` の候補として見る
...
9. `上流は通るが後ろで失敗` を見始めたら、動画URLをそのまま文字起こしする。最初は冒頭 20-45秒で十分。
...
- `上流は通るが後ろで失敗` を読むときは、`実際の冒頭文` を見ずに終わらない。広い危機訴求、著名人、無料特典などが `狙っている層を広く取っている` のか、`狙っていない層の比率を増やしている` のかを確認する。
...
- `同一CRで勝ち負け混在` の一覧を先に出し、家系分析より先に `運用差` を切る。
- 母数を広げた分析では、`失敗の件数が多い家系` と `失敗に偏る家系` を分けて読む。絶対件数だけで断定しない。
- `1年後悔` や `林さん` のような勝ち負け混在家系は、`家系そのものが失敗` ではなく `どの条件の個体が失敗したか` を読む。
